...
- Resolves the latest stable release tag and downloads matching `docker-compose.local.yml` and `.env.example`
- Pins `SUB2API_VERSION` in `.env` for reproducible upgrades and rollbacks
...
- **Displays generated credentials** (`POSTGRES_PASSWORD`, `REDIS_PASSWORD`, `ADMIN_PASSWORD`, `JWT_SECRET`, etc.)
...
# Upgrade after updating SUB2API_VERSION in .env
...
|----------|----------|---------|-------------|
| `SUB2API_VERSION` | Recommended | `0.1.103` | Pinned Sub2API image tag for reproducible deploys and easy rollback |
| `POSTGRES_PASSWORD` | **Yes** | - | PostgreSQL password |
| `REDIS_PASSWORD` | Recommended | *(empty)* | Redis password. Strongly recommended in production |
| `JWT_SECRET` | **Recommended** | *(auto-generated)* | JWT secret (fixed for persistent sessions) |
| `TOTP_ENCRYPTION_KEY` | **Recommended** | *(auto-generated)* | TOTP encryption key (fixed for persistent 2FA) |
| `SERVER_PORT` | No | `8080` | Server port |
| `SERVER_FRONTEND_URL` | Recommended | *(empty)* | Public base URL of the deployment, useful behind reverse proxy/CDN |
| `ADMIN_EMAIL` | No | `admin@sub2api.local` | Admin email |
| `ADMIN_PASSWORD` | Recommended | *(auto-generated by script)* | Admin password |
...
> **Note:** The `docker-deploy.sh` script automatically generates `JWT_SECRET`, `TOTP_ENCRYPTION_KEY`, `POSTGRES_PASSWORD`, `REDIS_PASSWORD`, and `ADMIN_PASSWORD` for you.
